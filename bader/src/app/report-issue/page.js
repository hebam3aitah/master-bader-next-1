...
export default function ReportProblemPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const reportData = {
        ...formData,
        images: formData.images.map((img) => img.name), // لاحقًا سيتم رفعها
      };

      const response = await axios.post('/api/r0eport', reportData);
  ...
  <div className="grid grid-cols-3 gap-4">
    {[
      { label: 'منخفضة', value: 'low', color: 'green-500', textColor: 'text-green-600' },
      { label: 'متوسطة', value: 'medium', color: '[#fa9e1b]', textColor: 'text-[#fa9e1b]' },
      { label: 'عالية', value: 'high', color: 'red-500', textColor: 'text-red-600' },
    ].map((option) => (
      <div key={option.value} className="relative">
        ...
        >
          <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mb-2 ${
            formData.severityLevel === option.value ? `border-${option.color} bg-${option.color}` : 'border-gray-300'
          }`}>
          ...
          <span className={`${option.textColor} font-medium`}>{option.label}</span>
        </label>
      </div>
    ))}
  </div>
                  ...
                  <div className="mt-8">
                    <div className="flex items-start mb-6">
                      ...
                      <label htmlFor="terms" className="mr-2 text-sm font-medium text-gray-700 text-right">
                        أتعهد بأن جميع المعلومات المقدمة صحيحة وأوافق على <a href="/termsCategories" className="text-[#fa9e1b] hover:underline">شروط الخدمة</a>
                      </label>
                    </div>
